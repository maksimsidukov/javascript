...
var max_bomb = 5; // Максимальное количество бомб на поле
var bomb = []; //Массим с бомбами

function init() {
    prepareGameField(); // Генерация поля
    document.getElementById('score').innerText = 'Счет 0:'; // Добалено! 
...
function move() {
    ...
    //Добавлено, ходим через стену
    if (new_unit == undefined){
        console.log('Уперлись в стену!');
        console.log(FIELD_SIZE_Y - 1)
        console.log(coord_x)
        if(direction == 'x-'){
    ...
    if (!isSnakeUnit(new_unit) && !haveBomb(new_unit)) { // Добалвена проверка на бомбу!
...
function createFood() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        // проверка на змейку
        if (!food_cell_classes.includes('snake-unit') && !haveBomb(food_cell)) { // Добавлено, что бы еда не создалась на бомбе
            var classes = '';
...
function createBomb() {
    ...
    bomb.push(bomb_cell);
    console.log(bomb);
    if(bomb.length > max_bomb){
        console.log('Удаляем бомбу!')
        var delete_bomb = bomb.shift();
        var delete_bomb_classes = delete_bomb.getAttribute('class').split(' ');
        delete_bomb.setAttribute('class', delete_bomb_classes[0]+' '+delete_bomb_classes[1]);
    }
}
/** ДОБАВЛЕНО
 * Проверка на бомбу
